    ...
    std::vector<RuntimeHelper> helpers = {
        {"CPPPContainerCompare", {}, {}, {}},
        {
            "CPPPFunctionType",
            {
                ...
                "    template <typename F, enable_if_t<!is_same<decay_t<F>, CPPPFunction>::value, int> = 0>",
                "    CPPPFunction(F&& value, string key) : closure(new function<R(Args...)>(std::forward<F>(value))), semanticKey(std::move(key)) {}",
                ...
                "        if (!left.semanticKey.empty() || !right.semanticKey.empty()) return left.semanticKey == right.semanticKey;",
