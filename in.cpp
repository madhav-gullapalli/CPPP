...
class CPPPClosureState { public: virtual ~CPPPClosureState() = default; virtual bool equals(const CPPPClosureState& other) const = 0; };
template <typename T> bool CPPPClosureEqual(const T& left, const T& right) { return left == right; }
template <typename Tuple, size_t... I> bool CPPPTupleEqual(const Tuple& left, const Tuple& right, index_sequence<I...>) { return (... && CPPPClosureEqual(get<I>(left), get<I>(right))); }
template <typename Callable, typename... Bound> class CPPPPartialClosureState final : public CPPPClosureState { Callable callable; tuple<Bound...> bound; public: CPPPPartialClosureState(Callable callable, Bound... bound) : callable(std::move(callable)), bound(std::move(bound)...) {} bool equals(const CPPPClosureState& other) const override { const auto* right = dynamic_cast<const CPPPPartialClosureState*>(&other); return right && CPPPClosureEqual(callable, right->callable) && CPPPTupleEqual(bound, right->bound, index_sequence_for<Bound...>{}); } };
...
class CPPPFunction<R(Args...)> {
    ...
    CPPPClosureState* closureState = nullptr;
public:
    ...
    template <typename F, enable_if_t<!is_same<decay_t<F>, CPPPFunction>::value, int> = 0>
    CPPPFunction(F&& value, CPPPClosureState* state) : closure(new function<R(Args...)>(std::forward<F>(value))), closureState(state) {}
    ...
    friend bool operator==(const CPPPFunction& left, const CPPPFunction& right) {
        ...
        if (left.closureState || right.closureState) return left.closureState && right.closureState && left.closureState->equals(*right.closureState);
...
template <typename A, typename B> class CPPPPair; template <typename T> class CPPPList; template <typename T> class CPPPHeap; template <typename T> class CPPPSet; template <typename K, typename V> class CPPPMap; struct CPPPRange;
...
template <typename T> void CPPPPrintValue(ostream& output, const CPPPHeap<T>& values);
...
void CPPPPrintValue(ostream& output, const CPPPRange& values);
...
template <typename T> void CPPPPrintValue(ostream& output, const CPPPHeap<T>& values) { output << "[*"; for (const auto& value : values.raw_items()) { output << ','; CPPPPrintValue(output, value); } output << ']'; }
...
void CPPPPrintValue(ostream& output, const CPPPRange& values) { output << '['; bool first = true; for (long long value : values) { if (!first) output << ", "; first = false; output << value; } output << ']'; }
...
template <typename T> struct CPPPDeepCopier<CPPPStack<T>> { static CPPPStack<T> run(const CPPPStack<T>& value) { CPPPStack<T> result; CPPPList<T> items = value.to_list(); for (const auto& item : items) result.push(CPPPCopy(item)); return result; } };

template <typename T> struct CPPPDeepCopier<CPPPQueue<T>> { static CPPPQueue<T> run(const CPPPQueue<T>& value) { CPPPQueue<T> result; CPPPList<T> items = value.to_list(); for (const auto& item : items) result.push(CPPPCopy(item)); return result; } };

template <typename T> struct CPPPDeepCopier<CPPPDeque<T>> { static CPPPDeque<T> run(const CPPPDeque<T>& value) { CPPPDeque<T> result; CPPPList<T> items = value.to_list(); for (const auto& item : items) result.push_back(CPPPCopy(item)); return result; } };

template <typename T> struct CPPPDeepCopier<CPPPHeap<T>> { static CPPPHeap<T> run(const CPPPHeap<T>& value) { CPPPHeap<T> result = value.copy_empty(); for (const auto& item : value.raw_items()) result.push(CPPPCopy(item)); return result; } };

template <typename T> struct CPPPDeepCopier<CPPPSet<T>> { static CPPPSet<T> run(const CPPPSet<T>& value) { CPPPSet<T> result = value.copy_empty(); for (const auto& item : value) result.insert(CPPPCopy(item)); return result; } };

template <typename K, typename V> struct CPPPDeepCopier<CPPPMap<K,V>> { static CPPPMap<K,V> run(const CPPPMap<K,V>& value) { CPPPMap<K,V> result = value.copy_empty(); for (const auto& item : value) result[CPPPCopy(item.first)] = CPPPCopy(item.second); return result; } };
...
int main() {
    ...
    long long __cppp_rep_limit_0 = 0;
    if (__cppp_rep_limit_0 < 0) throw runtime_error("1:1:rep count cannot be negative");
    for (long long __cppp_rep_0 = 0; __cppp_rep_0 < __cppp_rep_limit_0; ++__cppp_rep_0) {
        CPPPPrintValue(cout, CPPPStringLiteral("BAD")); cout << '\n';
    }
    if (__cppp_loop_completed_0) {
        CPPPPrintValue(cout, CPPPStringLiteral("yes")); cout << '\n';
    }
    return 0;
        ...
        if (__cppp_first != string::npos && __cppp_second != string::npos) {
            ...
            cout << "CP++:" << __cppp_line << ':' << __cppp_column << ": runtime error: " << __cppp_message.substr(__cppp_second + 1) << '\n';
        } else {
