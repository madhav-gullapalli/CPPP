...
WORK_DIR = ROOT / "tests" / "tmp" / "ast_invariants"


def main() -> int:
    parser = argparse.ArgumentParser(description="Stress the syntax-only ProgramAst pass.")
    ...
    for progress, (_, case) in enumerate(chosen, start=1):
        ...
        command = [str(COMPILER), "--cppp", str(source.relative_to(ROOT)), "--ast"]
        ...
        if first.returncode != 0:
            ...
            problem = "AST serialization changed between identical parses"
        elif not first.stdout.startswith("Program ["):
            problem = "AST output has no Program root"
        elif "UnknownStmt" in first.stdout or "UnknownExpr" in first.stdout:
            problem = "AST output contains an unknown node"
        elif "ErrorStmt" in first.stdout or "ErrorExpr" in first.stdout:
            problem = "valid source required syntax recovery"
        elif generated.exists():
            problem = f"--ast emitted {generated.relative_to(ROOT)}"
        if problem:
    ...
    if failures:
        print(f"\nAST invariant stress found {len(failures)} failure(s):", file=sys.stderr)
        for failure in failures:
            print(f"\n{failure}", file=sys.stderr)
        return 1

    print(f"All {args.count} deterministic AST cases passed (seed={args.seed}).")
    return 0
